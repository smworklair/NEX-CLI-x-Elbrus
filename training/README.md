...
`vliw/learned/scheduler.py` уже реализован и работает (`/learned` в CLI,
веса опубликованы — см. [vliw/learned/README.md](../vliw/learned/README.md)).
Этот документ — не про то, как заполнить пустоту, а про то, как повторить
или продолжить обучение: своя база, другой датасет, ещё эпохи. Модель учится
на парах «граф зависимостей → оптимальное расписание», эталон для которых
даёт оракул точного поиска, уже реализованный в проекте.
...
`vliw/learned/scheduler.py` уже умеет остальное — сам код для НОВОГО адаптера
менять не нужно, механизм готов: он кодирует `(dag, model)` через
`training.encode.encode_prompt`, даёт модели сгенерировать продолжение,
разбирает ответ `training.encode.decode_completion`, собирает `Schedule`,
вызывает `.validate()` — и только если чисто, возвращает `SchedulingResult`.
Если невалидно — честно откатывается на `baseline`
(GreedyListScheduler), а не выдаёт сломанное расписание как рабочее.
