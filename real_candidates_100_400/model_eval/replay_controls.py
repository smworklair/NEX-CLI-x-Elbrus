"""L3-реплей + decode-статистика на 12 контрольных графах (для V5).

Тот же код пути, что replay.py (decode_completion -> clip -> repair-зеркало
_assemble -> variants/choose + order-кандидат). Ожидание: победитель всегда
«жадный» (в портфеле он законен на нижней границе — ниже её нет ничего).
Выход: layers/l3_controls.json.
"""
...
for name in controls:
    ...
    n_dup_ids = (len(list(_LINE_RE.finditer(raw)))
                 - len({int(mm.group(1))
                        for mm in _LINE_RE.finditer(raw)}))
    ...
    wms = best.makespan if best.schedule.complete else None
    if wms is not None and wms < greedy.makespan:
        bad.append((name, wms, greedy.makespan))
    out[name] = {
        "nodes": n, "greedy": greedy.makespan,
        ...
        "decode": {
            ...
            "n_matches_total": len(list(_LINE_RE.finditer(raw))),
            "n_dup_ids": n_dup_ids,
            ...
            "ctx_truncated": trunc},
        "raw": {"placed": len(keep), "legal_errs": len(raw_errs),
                "complete": sched.complete,
                "makespan": (sched.makespan
                             if sched.complete and not raw_errs else None)},
        "repaired_touched": touched,
        "portfolio": [{"name": c.name, "legal": c.legal,
                       "makespan": c.makespan if c.schedule.complete else None}
                      for c in cands],
        "winner": best.name, "winner_ms": wms,
    ...
    print(f"{name:20s} placed={len(keep):4d}/{n} cov={len(keep)/n:.2f} "
          f"winner={best.name}:{wms} order={s_ord.makespan} L1={s_rule.makespan}",
          flush=True)

pathlib.Path(ME + "/layers").mkdir(parents=True, exist_ok=True)
json.dump(out, open(ME + "/layers/l3_controls.json", "w"),
          ensure_ascii=False, indent=1)
print("CONTROL VIOLATIONS (L3 < greedy):", bad if bad else "none")
assert not bad
