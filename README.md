...
```
      ...
      ██   ███  ███████  ██    ██       ▄█████████████████████████████████▄
                   ...
                   NEX CLI  ·  Elbrus e2k
              четыре инструмента — у каждого свой экран
...
лету: в гуттере у строки стоит её такт и канал (`т7 ,0`), ошибки машины
(чужой канал, занятый порт, чтение раньше латентности) помечаются сразу,
без всякого F5. `F5` запускает дорогое — точный поиск и диагноз; после него
в гуттере появляется `т7→1`: куда ту же строку кладёт оракул. `F6`
переписывает буфер по расписанию точного поиска — те же мнемоники и
операнды, изменены только каналы и разбивка на широкие команды. Внизу
строка состояния: слева разбор строки под курсором (клик открывает вкладку
ТАКТЫ), справа живые счётчики замечаний и git — как строка состояния в
IDE, клик по пункту открывает нижнюю панель на нужной вкладке; `F12`
открывает и убирает панель целиком (ЗАМЕЧАНИЯ / GIT / ТАКТЫ).
...
объём вывода). Ряд **«мостик ▸»** стоит прямо во вкладке GIT, без всякого
...
Во всех четырёх режимах одинаково: `/` открывает палитру команд (стрелки листают,
...
vliw/
  ...
  tui/             ПОЛНОЭКРАННЫЙ ИНТЕРФЕЙС (Textual; единственный fullscreen)
    screens/         четыре рабочих места: ЯДРО / РАЗБОР / АГЕНТ / КОД
