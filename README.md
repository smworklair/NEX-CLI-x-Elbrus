...
```
 ▍ NEX   КОД   редактор      15 т.  ·  15 оп.  ·  e2k-v6-measured
 slots.s ✕  probe.s ✕    ассемблер e2k (.s)          ← вкладки открытых файлов
 примеры машины  ▏ ▷   11  т0   muls,0 %r10, %r11, %r20
  slots          ▏     12       }
  divport        ▏     13       {                          ← редактор, 60–70%
 настоящий код   ▏     14  т1   muls,0 %r12, %r13, %r21
  probe.s        ▏     15       }
─────────────────────────────────────────────────────────────────────────
 расписание  разбор  замечания  вывод  ядро    15 т.  как написано  оракул
  т0      т1      т2      т3      т4     ▏ ПОРТЫ ,0:8 ,1:4 ,2:2 ,3:1
 ,0 muls    muls    muls    muls    muls  ▏ слоты 17%          ← ОДНО окно
 ,1 ·       ·       ·       ·       ·     ▏ ПЕРЕСТАВИТЬ  6        с вкладками
 стр.11  MUL  лат.4  т0 ▸   15 оп. · исходник 15 т. → поиск 8 т.  ✓ чисто
  F5 прогнать   F12 док ↓   F11 развернуть док   ^P команда   ^G объяснить
```
...
`15 т.`, `3 ошибки`, `7 коммитов`. Всегда понятно, что это за область и
откуда там содержимое.

**Панели независимы.** `F12` убирает нижний док целиком — редактор занимает
весь экран, и ничего при этом не ломается. `^B` убирает каталог слева.
`F11` (или двойной клик по полосе вкладок) разворачивает док на две трети:
вкладка получает то, чему в трети экрана места нет — вторую решётку рядом с
первой, полные тексты замечаний, журнал сессии вместо ленты вывода.
...
пример, открытый поверх буфера, съедал бы написанное. Каталог слева делит
...
и памятью). Внизу одна строка состояния: слева разбор строки под курсором,
справа итог по буферу и живые счётчики замечаний и git — клик по пункту
открывает свою вкладку. `^G` — объяснятель: спросить агента про то, на что
