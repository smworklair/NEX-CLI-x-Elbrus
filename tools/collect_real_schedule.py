#!/usr/bin/env python3
"""Собрать анонимизированный граф зависимостей из настоящего .s-вывода lcc.

ЭТО ПИСЬМО-ВЛОЖЕНИЕ, А НЕ ЧАСТЬ ПРОЕКТА. Если тебе прислали этот файл —
запускать нужно только его, весь остальной репозиторий не нужен и не
требуется. Единственная зависимость — стандартная библиотека Python 3.

    python3 collect_real_schedule.py путь/к/файлу.s

Что делает: читает вывод компилятора e2k (широкие команды в фигурных
скобках, `adds,0 %r1, %r2, %r3` и т.п.), строит из него граф зависимостей
между операциями и — если в файле явно указаны каналы — восстанавливает
расписание, которое построил сам компилятор. Результат печатается ОДНОЙ
JSON-строкой в стандартный вывод. НИКУДА САМ НЕ ОТПРАВЛЯЕТ НИЧЕГО — ни по
сети, ни на диск, если не указан --out. Что делать с распечатанной строкой
(сохранить, приложить к письму, выбросить) — решаешь ты, глядя на неё.

ЧТО ПОПАДАЕТ В РЕЗУЛЬТАТ. Только структура: класс операции (сложение,
умножение, загрузка, ...), кто от кого зависит, в каком такте и на каком
канале её разместил компилятор. НЕ попадает: текст исходных строк, имена
файлов кроме короткого базового имени, комментарии, что именно программа
вычисляет. Номера регистров заменяются на локальные порядковые номера
(0, 1, 2...) — оригинальные имена регистров тоже никуда не идут.

Перед тем как что-то отправлять — прочитай распечатанное. Оно всё
человекочитаемо: `граф:` показывает ровно то, что уйдёт наружу.

Зачем это вообще нужно (контекст для тех, кому прислали файл): проект
учит модель предсказывать расписания для VLIW-конвейера e2k на СИНТЕТИЧЕСКИХ
графах, посчитанных генератором. Мы не знаем, насколько их форма похожа на
графы из настоящего кода. Несколько реальных примеров (даже 5-10) сильно
помогли бы — сравнить синтетику с реальностью, ничего больше.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.quiet:
        print(f"\nНичего никуда не отправлено само — файл{'ы' if len(args.files)>1 else ''} "
             ...
             + ". Отправлять или нет — решать тебе.")
    return 0
