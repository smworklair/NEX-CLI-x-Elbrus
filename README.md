...
`python3 -m vliw` без аргументов открывает **КОД** — редактор ассемблера e2k
со всем остальным вкладками его нижнего дока. Один экран и пристыкованные
окна, как в IDE:
...
```
 ...
 ▷   11  т0     muls,0 %r10, %r11, %r20
     12         }
     13         {                                          ← редактор, 60–70%
     14  т1→0   muls,0 %r12, %r13, %r21
     15         }
──────────────────────────────────────────────────────────────────────────────
  расписание   разбор   замечания   вывод   ядро    15 т. −7  как написано  оракул
    т0      т1      т2      т3      т4      т5      т6      т7      т8      т9
 ,0  muls    muls    muls    muls    muls    muls    muls    muls    ·       ·
 ,1  ·       ·       ·       ·       ·       ·       ·       ·       adds    adds
 ,2  ·       ·       ·       ·       ·       ·       ·       ·       ·       ·
 стр.11  MUL лат.4  т0 ▸        слоты 17% → 31%  ·  резерв 7 т. (47%)  ✓ чисто  git 1
  F5 прогнать   F6 переписать   F12 док   ^P команда   ^G спросить
```

**Снизу одно окно, а не два.** Пока их было два — расписание отдельной
панелью и выдвижной ящик под ней, — коду на терминале 150×42 доставалось
одиннадцать строк из сорока двух, меньше, чем каждому из окон под ним.
Теперь окно одно, с вкладками, и у каждой есть имя и живой итог справа:
`15 т. −7`, `3 ошибки`, `7 коммитов`. Всегда понятно, что это за область и
откуда там содержимое.

**Каждое число живёт в одном месте.** Наверху — «сколько и во сколько»
(`15→8 т. · 15 оп.`), внизу — «что с этим делать» (`слоты 17% → 31% ·
резерв 7 т.`). Строк наверху была две, и вторая повторяла числа из первой;
строка состояния повторяла их же в третий раз. Одно число в трёх местах
экрана — это не подстраховка, а шум.

**Панели независимы, и по умолчанию их нет.** `F12` убирает нижний док
целиком — редактор занимает весь экран, ничего не ломая. `^B` открывает
каталог файлов слева (закрыт: нужен он в первую минуту и когда открываешь
второй файл, а место отбирал бы всегда). `F11` — или двойной клик по полосе
вкладок — разворачивает док на две трети, и вкладка получает то, чему в
трети экрана места нет: вторую решётку рядом с первой, загрузку портов,
полные тексты замечаний, журнал сессии вместо ленты вывода.

**Рамок нет.** Три панели в рамках стоили шесть строк на одни бордюры и
печатали свои имена капсом там, где место нужно содержимому. Границ ровно
две: волосок над доком и подложка строки состояния. Строка состояния одна и
стоит в самом низу — раньше она висела между расписанием и ящиком, посреди
экрана.

**Действие живёт у строки.** `▷` в гуттере на строке под курсором прогоняет
буфер — то же, что `F5`. Ряда текстовых кнопок над кодом больше нет.

**Канал не уезжает.** В решётке `,0`…`,5` — закреплённая колонка: тактов
много, вбок прокручивают всегда, и подпись, уезжающая вместе с
содержимым, лишила бы решётку её единственного ответа.

Остальные экраны (**РАЗБОР** — весь граф без исходника, с матрицей машины и
диагнозом; **АГЕНТ** — разговор; полноэкранное **ЯДРО** — лента с коммитами
и снимками машины) открываются по `/clear` или `^O` и возвращают обратно по
`Esc`. Равноправными режимами они быть перестали: к ним уходят из работы, а
не начинают ими. Цифры работают клавишами только на этом списке — внутри
экрана вся клавиатура принадлежит редактору и строке ввода.

**ЯДРО — верстак интерпретатора.** Короткая версия есть вкладкой в КОДЕ
(консоль, имена, память, размер графа) — её хватает, чтобы собрать граф, не
умея писать ассемблер. Полноэкранное ЯДРО добавляет то, чего во вкладку не
влезает: ленту вычислений с коммитами и снимками машины. Слева лента, справа три панели
...
**КОД — редактор, который знает, что в нём написано.** Буфер разбирается на
лету: в гуттере у строки стоит её такт (`т7`), ошибки машины (чужой канал,
занятый порт, чтение раньше латентности) помечаются сразу, без всякого F5.
`F5` запускает дорогое — точный поиск и диагноз; после него в гуттере
появляется `т7→1`: куда ту же строку кладёт оракул. `F6` переписывает буфер
по расписанию точного поиска — те же мнемоники и операнды, изменены только
каналы и разбивка на широкие команды.

Открытых файлов может быть несколько, у каждого своя вкладка: сравнение
двух участков — своего и того, что выдал lcc, — здесь основная работа, а
пример, открытый поверх буфера, съедал бы написанное. Каталог по `^B` делит
файлы на две ветки — «примеры машины» (учебные участки, у каждого своя
дыра) и «настоящий код» (`examples/*.s`, вывод lcc): разница между
показательным и настоящим здесь главная, и смешивать их значило бы её
спрятать.

Вкладки нижнего дока: **расписание** (каналы по строкам, такты по
колонкам, прокрутка вбок — расписание разрежено, и длинное оно вдоль
времени), **разбор** (такт под курсором, его цепочка зависимостей, каталог
тактов буфера), **замечания**, **вывод** (прогоны, история сессии, мостик в
другие экраны, строка команд) и **ядро** (консоль интерпретатора с именами
и памятью). Внизу одна строка состояния: слева строка под курсором,
справа занятость слотов, резерв в тактах и живые счётчики замечаний и git —
клик по счётчику открывает свою вкладку. `^G` — объяснятель: спросить агента про то, на что
смотришь; фокус в редакторе значит вопрос про код, открытая вкладка — про
её содержимое.

**GIT — история сессии и мостик между режимами.** Журнал команд один на все
экраны: что бы вы ни запускали в ЯДРЕ, РАЗБОРЕ, АГЕНТЕ или КОДЕ, запись
ложится в **ИСТОРИЮ** — как коммиты в git log: точка графа цветом режима,
номер, время, команда, а второй строкой состояние (участок, размер графа,
объём вывода). Ряд **«мостик ▸»** стоит прямо во вкладке ВЫВОД, без всякого
разворота: выбрал запись кликом в истории — отправил. В развороте журнал
добавляет каталог всех команд слева, историю по `↑↓` и повтор по `^R`.
«В разбор» делает запись текущим участком, «в агента»
готовит вопрос, «в ядро»
открывает верстак с этим контекстом, «в код» возвращает исходник прогона
в буфер редактора; «вернуть» восстанавливает состояние сессии той записи
(участок, исходник) прямо на месте — checkout без перехода. Полная сетка:
из любого режима запись доезжает до любого другого. Ничто не уезжает
само: что сделать общим по сессии, решаете вы. Живой поток (строки модели,
пометки планировщика) идёт отдельной вкладкой СОБЫТИЯ и не перемешивается
с отчётами команд.

На всех экранах одинаково: `/` открывает палитру команд (стрелки листают,
`Tab` подставляет, `Enter` выполняет, `Esc` закрывает), `↑↓` при закрытой
палитре — история ввода, `/clear` — к списку экранов, `/exit` — выход. Долгий
счёт (точный поиск) идёт в отдельном потоке: экран не замирает.

Вспомогательные панели убираются с клавиатуры, когда мешают: `^B` прячет
боковую колонку (в КОДЕ — каталог файлов), `^T` — полосу подсказок с
ядрами, участками и вопросами, `F12` в КОДЕ — весь нижний док. Убранная
панель ничего не ломает: остальное продолжает работать, просто шире.
...
Без неё инструмент запускается в построчном режиме. В пайпе, при
перенаправлении в файл или с флагом `--plain` — тоже: те же команды и тот же
вид отчётов. Разовые вызовы (`python3 -m vliw run …`) полноэкранный режим
...
печатают список команд по группам; `/help <команда>` — подробно об одной;
`/exit` (или `exit`, Ctrl+D) — выход.
