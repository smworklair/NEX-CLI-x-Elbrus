...
- Модель машины игрушечная: нет регистрового давления, спекуляции,
  ...
  нет ограничений на комбинации операций внутри одной широкой команды.
...
1. Проверить латентности на ЖИВОМ железе, а не только по вере компилятора
   ...
   снята надёжно — у ассемблера; под вопросом именно тайминги.
2. Перенести постановку на Hexagon в LLVM: там открытый `MachineScheduler`,
   реальное описание слотов широкой команды и доступ к внутренностям.
3. Обучать модель на парах «граф → расписание», где эталон даёт оракул из
